...
for Instance = 1%:8
    ...
    if strcmp(Session, 'mua')
        ...
        if strcmp(Monkey,'lick')
            RESP0=load(fullfile(datafld,Session,['Lick_20180807_B2_array_' num2str(Instance) '_medMUA.mat']));
            C0=RESP0.C;
        elseif strcmp(Monkey,'aston')
            RESP0=load(fullfile(datafld,Session,['Aston_20181004_B1_array_' num2str(Instance) '_medMUA.mat']));
            C0=RESP.C;
        end
        
        if strcmp(Monkey,'lick')
            RESP=load(fullfile(datafld,Session,['Lick_20180807_B2_array_' num2str(Instance) '_medMUA_odd.mat']));
            C=RESP.C;
            RESP2=load(fullfile(datafld,Session,['Lick_20180807_B2_array_' num2str(Instance) '_medMUA_even.mat']));
            RESP.mMUA_even=RESP2.mMUA_even;
        elseif strcmp(Monkey,'aston')
            RESP=load(fullfile(datafld,Session,['Aston_20181004_B1_array_' num2str(Instance) '_medMUA_odd.mat']));
            C=RESP.C;
            RESP2=load(fullfile(datafld,Session,['Aston_20181004_B1_array_' num2str(Instance) '_medMUA_even.mat']));
            RESP.mMUA_even=RESP2.mMUA_even;
        end
...
%% save the results
save(['Check_' Session '_median'],'ephys_data','ephys_data0')

%% Plot the odd and even response profiles
% [~,~] = mkdir(['Check_' Session '_median']);
% cd(['Check_' Session '_median']);
% 
% for elec=1:1024
...
[~,~] = mkdir(['Check2_' Session '_median']);
cd(['Check2_' Session '_median']);
