...
if Do.SaveMUA_perArray
    ...
    for m=1:length(M) %>> while testing, do only 1 array
        ...
        for c=1:length(M(m).chan)
            ...
            M(m).ch(c).bars = [];  M(m).ch(c).bars_odd = [];  M(m).ch(c).bars_even = [];
            ...
            for r=1:length(N(m).run)
                ...
                barresp=[];barresp_odd=[];barresp_even=[];
                for b=1:length(N(m).run(1).stim_sec)
                    ...
                    barresp = [barresp mean(M(m).ch(c).coll(r,t1i:t2i)-...
                        mean(M(m).ch(c).BL(r,:)))];
                    if mod(r,2) % odd
                        rr=size(M(m).ch(c).bars_odd,1)+1;
                        barresp_odd = [barresp_odd mean(M(m).ch(c).coll_odd(rr,t1i:t2i)-...
                            mean(M(m).ch(c).BL_odd(rr,:)))];
                    else % even
                        rr=size(M(m).ch(c).bars_even,1)+1;
                        barresp_even = [barresp_even mean(M(m).ch(c).coll_even(rr,t1i:t2i)-...
                            mean(M(m).ch(c).BL_even(rr,:)))];
                    end
                end
                M(m).ch(c).bars = [M(m).ch(c).bars; barresp];  
                M(m).ch(c).bars_odd = [M(m).ch(c).bars_odd;barresp_odd];  
                M(m).ch(c).bars_even = [M(m).ch(c).bars_even;barresp_even];

            end
            mMUA(c).bar = median(M(m).ch(c).bars);
            mMUA_odd(c).bar = median(M(m).ch(c).bars_odd);
            mMUA_even(c).bar = median(M(m).ch(c).bars_even);
