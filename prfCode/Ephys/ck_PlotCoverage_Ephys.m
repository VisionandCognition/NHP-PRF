...
for m = 1:2
    for VA = 1:3
        ...
        for a=unique(RetMap(m).table_mua.Array)'
            if AreaName{1,VA} % not zero
                inc = RetMap(m).table_mua.Array == a & RetMap(m).table_mua.prf_r2 > rth & ...
            ...
            figure(f(m,1));
            subplot(4,4,a); hold on; box on;
            plot([0 0],[-100 100],'k','linewidth',.5);
            plot([-100 100],[0 0],'k','linewidth',.5);
            viscircles([RetMap(m).table_mua.prf_x(inc),RetMap(m).table_mua.prf_y(inc)],...
                RetMap(m).table_mua.prf_sd(inc)./2,'Color','k','LineWidth',1);
            ...
            title([RetMap(m).Subj ': Array ' num2str(a) ', Area: ' AN ' [MUA]'],'FontSize', 18);
