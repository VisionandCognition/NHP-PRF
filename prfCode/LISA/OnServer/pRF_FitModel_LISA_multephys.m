function pRF_FitModel_LISA_multephys(Monkey,Session,Instance,numWorkers,modeltype,cv,resfld)
...
result_folder = fullfile(mlroot, 'Results_AVG', Monkey, resfld, ['Instance_' InstanceLabel]);
...
if strcmp(Monkey,'lick')
    load(fullfile(datafld,'stim','Lick_STIM.mat'))
elseif strcmp(Monkey,'aston')
    load(fullfile(datafld,'stim','Aston_STIM.mat'))
end
...
if strcmp(Session, 'mua')
    ...
    if cv == 0 % no crossvalidation
        if strcmp(Monkey,'lick')
            RESP=load(fullfile(datafld,Session,['Lick_AVG_array_' num2str(Instance) '_mMUA.mat']));
            C=RESP.C;
        elseif strcmp(Monkey,'aston')
            RESP=load(fullfile(datafld,Session,['Aston_AVG_array_' num2str(Instance) '_mMUA.mat']));
            C=RESP.C;
        end
    elseif cv == 1 % crossvalidation
        if strcmp(Monkey,'lick')
            RESP=load(fullfile(datafld,Session,['Lick_AVG_array_' num2str(Instance) '_mMUA_odd.mat']));
            C=RESP.C;
            RESP2=load(fullfile(datafld,Session,['Lick_AVG_array_' num2str(Instance) '_mMUA_even.mat']));
            RESP.mMUA_even=RESP2.mMUA_even;
        elseif strcmp(Monkey,'aston')
            RESP=load(fullfile(datafld,Session,['Aston_AVG_array_' num2str(Instance) '_mMUA_odd.mat']));
            C=RESP.C;
            RESP2=load(fullfile(datafld,Session,['Aston_AVG_array_' num2str(Instance) '_mMUA_even.mat']));
            RESP.mMUA_even=RESP2.mMUA_even;
        end
    end
...
elseif strcmp(Session,'lfp')
    ...
    if cv == 0 % no crossvalidation
        if strcmp(Monkey,'lick')
            RESP=load(fullfile(datafld,Session,['Lick_AVG_array_' num2str(Instance) '_mLFP.mat']));
            C=RESP.C;
        elseif strcmp(Monkey,'aston')
            RESP=load(fullfile(datafld,Session,['Aston_AVG_array_' num2str(Instance) '_mLFP.mat']));
            C=RESP.C;
        end        
    elseif cv == 1 % crossvalidation
        if strcmp(Monkey,'lick')
            RESP=load(fullfile(datafld,Session,['Lick_AVG_array_' num2str(Instance) '_mLFP_odd.mat']));
            C=RESP.C;
            RESP2=load(fullfile(datafld,Session,['Lick_AVG_array_' num2str(Instance) '_mLFP_even.mat']));
            RESP.mLFP_even=RESP2.mLFP_even;
        elseif strcmp(Monkey,'aston')
            RESP=load(fullfile(datafld,Session,['Aston_AVG_array_' num2str(Instance) '_mLFP_odd.mat']));
            C=RESP.C;
            RESP2=load(fullfile(datafld,Session,['Aston_AVG_array_' num2str(Instance) '_mLFP_even.mat']));
            RESP.mLFP_even=RESP2.mLFP_even;
        end        
    end
