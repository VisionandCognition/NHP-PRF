function pRF_FitModel_LISA_ephys(Monkey,Session,Instance,numWorkers,modeltype,cv,resfld)
...
if strcmp(Session, 'mua')
    ...
    if cv == 0 % no crossvalidation
        if strcmp(Monkey,'lick')
            RESP=load(fullfile(datafld,Session,['Lick_20180807_B2_array_' num2str(Instance) '_medMUA.mat']));
            C=RESP.C;
        elseif strcmp(Monkey,'aston')
            RESP=load(fullfile(datafld,Session,['Aston_20181004_B1_array_' num2str(Instance) '_medMUA.mat']));
            C=RESP.C;
        end
